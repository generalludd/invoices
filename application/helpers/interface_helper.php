...
function create_edit_field($field_name, $value, $label, $options = array())
{
	...
	if ($label != "") {
		$output ['label'] = sprintf ( "<label>%s:&nbsp;</label>", $label );
	}

    if(array_key_exists('link',$options)) {
        $output['label'] = sprintf("<a href='%s'>%s</a>", base_url($options['link']), $output['label']);
    }
...
function internal_link($path, $text, $title = NULL){
	if(!$title){
		$title = $text;
	}
	return sprintf('<a href="%s" title="%s">%s</a>', base_url($path), $title, $text);
}
